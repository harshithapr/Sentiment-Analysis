...
def get_labels():
    label_map={1: 'truthful positive',2: 'truthful negative',3:'deceptive positive',4:'deceptive negative'}
    return label_map
def getTokens(line):
...
def read_modeldata():
    ...
    for prior_data in prior_dataset:
        ...
        prior_prob[int(tokens[0])]=float(tokens[1])
    #print(prior_prob)
...
def posterior_probability(prior_prob,like_prob,words):
    labels=[1,2,3,4]
    score = dict()
    for label in labels:
...
def maximum_likelihood(prior_prob,like_prob,reviews):
    ...
    for review in reviews:
        ...
        identifier = tokens[0]
        words=tokens[1:]
        score=posterior_probability(prior_prob, like_prob, words)
        # score2=posterior_probability(prior_prob, like_prob, label_sequence2, words)
        print(score)
        # print(score2)
        result_line=identifier+" "+get_labels()[max_prob(score)]
